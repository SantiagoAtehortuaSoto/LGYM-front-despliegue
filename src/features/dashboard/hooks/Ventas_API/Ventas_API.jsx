...
// src/features/dashboard/hooks/Ventas_API/Ventas_API.jsx
...
export const estadoToPayload = (valor) => {
  ...
  const idEntry = Object.entries(ESTADO_ID_MAP).find(
    ([, label]) => label === estado
  );
  return {
    estado,
    ...(idEntry ? { id_estado: Number(idEntry[0]) } : {}),
  };
};
...
const buildVentaPayload = (venta) => {
  const detallesOrigen = Array.isArray(venta.detalles)
    ? venta.detalles
    : [];

  const detalles = detallesOrigen.map(mapDetalleToBackend);

  const valor_total_venta =
    venta.valor_total_venta ??
    calcularTotalVenta(detalles);
  ...
  const fecha_venta =
    venta.fecha_venta ?? venta.fechaVenta ?? undefined;
  const plazo_maximo =
    venta.plazo_maximo ??
    venta.plazoMaximo ??
    venta.fecha_entrega ??
  ...
  const estado_venta =
    venta.estado_venta ?? venta.estadoVenta ?? undefined;
  // Si quieres forzar mayúsculas podrías hacer:
  // const estado_venta =
  //   (venta.estado_venta ?? venta.estadoVenta ?? "")
  //     .toString()
  ...
  return {
    ...
    ...(estado_venta ? { estado_venta } : {}), // si no viene, backend usará default PENDIENTE
    valor_total_venta,
    detalles,
  };
};
...
export const obtenerVentaPorId = async (id, options) => {
  ...
  return logAndFetch("[Ventas][GET detalle]", url, {
    method: "GET",
    headers: buildHeaders(token),
  });
};
...
export const crearVenta = async (venta, options) => {
  ...
  return logAndFetch("[Ventas][POST crear]", URL_VENTAS, {
    method: "POST",
...
export const actualizarVenta = async (id, venta, options) => {
  ...
  return logAndFetch("[Ventas][PUT actualizar]", url, {
    method: "PUT",
    headers: buildHeaders(token, { "Content-Type": "application/json" }),
    body: JSON.stringify(payload),
  });
